...
class Occluded_classifier(nn.Module):

    def __init__(self, in_dim, occluded_num):
        super(Occluded_classifier, self).__init__()

        self.in_dim = in_dim
        self.o_num = occluded_num

        self.classifier = nn.Linear(self.in_dim, self.o_num, bias=False)

        self.classifier.apply(weights_init_classifier)

    def forward(self, x):
        od_score = self.classifier(x.squeeze())
        return od_score
